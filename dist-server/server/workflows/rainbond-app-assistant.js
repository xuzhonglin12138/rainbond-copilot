...
function buildUiScopeFromSessionContext(sessionContext = {}) {
    ...
    return {
        teamName: typeof sessionContext.teamName === "string"
            ? sessionContext.teamName
            : typeof sessionContext.team_name === "string"
                ? sessionContext.team_name
                : undefined,
        regionName: typeof sessionContext.regionName === "string"
...
function isCapabilityPrompt(message) {
    return /(你能做什么|可以做什么|有哪些流程|有哪些能力|有哪些工作流|workflow|skill|技能)/i.test(message || "");
}
function isGenericIssueInspectionPrompt(message) {
    return /((这个|当前)?(组件|应用).*(怎么了|怎么回事|什么问题|有问题|出问题|啥情况))|((what'?s|what is).*(wrong|issue))|((component|app).*(wrong|issue))/i.test(message || "");
}
...
export async function executeRainbondAppAssistant(input) {
    ...
    if (/(修复|恢复服务|故障|异常|启动不起来|排查)/i.test(input.message) ||
