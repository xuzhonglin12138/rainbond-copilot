function isDebugEnabled() {
    const raw = process.env.COPILOT_DEBUG_WORKFLOW ||
...
function summarizeValue(value) {
    ...
    if (typeof value === "object") {
        ...
        if (record.status && typeof record.status === "object") {
            ...
            summary.status = Object.fromEntries(Object.entries(statusRecord).filter(([key, entryValue]) => {
                return (typeof entryValue === "string" ||
                    typeof entryValue === "number" ||
                    typeof entryValue === "boolean") && key !== "token" && key !== "password";
            }));
        }
        return summary;
    }
...
export function logWorkflowDebug(event, payload) {
    if (!isDebugEnabled()) {
        return;
    }
