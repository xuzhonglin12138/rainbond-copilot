...
export function createContinuationRouter(opts) {
    const tool = {
        ...
        function: {
            name: TOOL_NAME,
            description: "Choose how the current user message should relate to the previous workflow continuation.",
            parameters: {
                type: "object",
                properties: {
                    action: {
    ...
    return {
        async route(input) {
            const response = await opts.llmClient.chat([
                ...
                {
                    role: "user",
                    content: buildContinuationRouterMessage(input.message, input.continuation, input.sessionContext),
                },
            ], [tool]);
            ...
            if (action !== "reuse_continuation_context" &&
            ...
            return {
                action,
                option_key: typeof parsed.option_key === "string" && parsed.option_key
                    ? parsed.option_key
                    : undefined,
...
function buildContinuationRouterMessage(message, continuation, sessionContext) {
    const lines = [
        `当前用户消息：${message || "(空)"}`,
        "",
        "上一轮 workflow 上下文：",
        ...
        continuation.suggestedActions && continuation.suggestedActions.length > 0
            ? `- suggested_actions: ${JSON.stringify(continuation.suggestedActions.map((item) => ({
...
function pickRelevantContext(sessionContext) {
    const keys = [
        "teamName",
        "team_name",
        "regionName",
