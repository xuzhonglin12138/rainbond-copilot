...
const EMBEDDED_WORKFLOW_KNOWLEDGE = {
    "rainbond-app-assistant": {
        ...
        avoidWhen: "用户已经明确指定只执行某个子流程，或者任务属于 workspace 初始化 / 本地 env 同步。",
        preferredTools: [
    ...
    "rainbond-fullstack-troubleshooter": {
        useWhen: "拓扑已经存在，但运行态未收敛，需要低风险检查、归类阻塞点并尝试平台侧修复。",
...
export function buildEmbeddedWorkflowKnowledgeSection() {
    ...
    for (const workflow of workflowRegistry.list()) {
        ...
        if (metadata?.stages?.length) {
            lines.push(`- 流程阶段：${metadata.stages.map((stage) => `${stage.id}(${stage.label})`).join(" -> ")}`);
        }
        lines.push("");
    }
    lines.push("workspace 型流程 rainbond-project-init、rainbond-env-sync 不在当前嵌入式主链路内；如任务依赖本地项目文件初始化或本地 env 落盘，应明确提示这是外部/后续流程。");
    return lines.join("\n");
}
export function buildMcpToolUsageKnowledgeSection() {
    const directExecutionTools = MUTABLE_TOOL_POLICY_LIST.filter((item) => item.allowDirectExecution).map((item) => item.name);
    const approvalRequiredTools = MUTABLE_TOOL_POLICY_LIST.filter((item) => !item.allowDirectExecution).map((item) => item.name);
