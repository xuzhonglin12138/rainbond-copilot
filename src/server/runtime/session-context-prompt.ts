...
function buildWorkflowHints(input: {
  ...
  if (input.componentId) {
    ...
    if (componentSource === "route") {
      hints.push(
        "当前 component_id 来自 route 语境，可能不是可直接写入 MCP 的 service_id；在把它当成组件 service_id 前先验证。"
      );
    }
...
export function buildSessionContextPromptMessages(
  sessionContext?: Record<string, unknown>
): ChatMessage[] {
  ...
  const enterpriseId = readContextString(
    sessionContext,
    "enterpriseId",
    "enterprise_id"
  );
  const teamName = readContextString(sessionContext, "teamName", "team_name");
  const regionName = readContextString(
    sessionContext,
    "regionName",
    "region_name"
  );
  const appId = readContextString(sessionContext, "appId", "app_id");
  const componentId = readContextString(
    sessionContext,
    "componentId",
    "component_id"
  );
  const componentSource = readContextString(
    sessionContext,
    "componentSource",
    "component_source"
  );
