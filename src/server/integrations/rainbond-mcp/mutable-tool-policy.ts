...
const enterprise = "enterprise" as const;
...
export function getApprovalScopeLabel(scope?: ApprovalScope | null): string {
  if (!scope) {
    return "";
  }
...
export function getApprovalRiskLabel(risk?: RiskLevel | null): string {
  if (!risk) {
    return "";
  }
...
function buildSpecialApprovalMessage(
  toolName: string,
  input: Record<string, unknown>
): string {
  const regionName = readString(
    input.region_alias,
    input.region_name,
    input.name
  );
