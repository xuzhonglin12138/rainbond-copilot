...
function extractComponentName(actionResult: Record<string, any>): string | undefined {
  return (
    actionResult?.service?.component_name ||
...
function extractAppName(actionResult: Record<string, any>): string | undefined {
  return (
    actionResult?.app?.app_name ||
...
function extractPrimaryName(actionResult: Record<string, any>): string | undefined {
  return (
    extractComponentName(actionResult) ||
...
function extractLogLines(actionResult: Record<string, any>): string[] {
...
export function buildPendingWorkflowActionCompletion(
  ...
  const actionResult = (output.structuredContent || {}) as Record<string, any>;
  const toolStatus = output.isError ? "error" : "success";
  const subflowData: Record<string, unknown> = {};
  ...
  switch (pendingAction.toolName) {
    ...
    case "rainbond_install_app_model": {
      const installedAppName =
        actionResult?.installed_app_name || actionResult?.app_name;
    ...
    case "rainbond_get_component_logs": {
      ...
      const requestedLines =
        typeof pendingAction.arguments.lines === "number"
    ...
    case "rainbond_operate_app": {
      ...
      const serviceIds = Array.isArray(pendingAction.arguments.service_ids)
        ? pendingAction.arguments.service_ids.filter(
            (item): item is string => typeof item === "string" && !!item
          )
        : [];
      const appId =
        typeof pendingAction.arguments.app_id === "number"
    ...
    case "rainbond_change_component_image": {
      const componentName = extractPrimaryName(actionResult);
      const image =
        actionResult?.image || pendingAction.arguments.image || "目标镜像";
      summary = componentName
    ...
    case "rainbond_manage_component_connection_envs": {
      const componentName =
        ...
        (typeof pendingAction.arguments.service_id === "string"
          ...
          : undefined);
      const attrName =
        typeof pendingAction.arguments.attr_name === "string"
    ...
    case "rainbond_manage_component_ports": {
      ...
      const operation =
        typeof pendingAction.arguments.operation === "string"
          ...
          : "";
      const port =
        typeof pendingAction.arguments.port === "number"
    ...
    case "rainbond_check_helm_app": {
      const name =
        (typeof actionResult?.name === "string" && actionResult.name) ||
