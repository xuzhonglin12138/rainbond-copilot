...
import type {
  ...
  CompiledMachineBlockKind,
  CompiledSkill,
  CompiledSkillMode,
  CompiledWorkflowDefinition,
} from "./compiled-types.js";

interface ParsedMachineBlock {
  kind: CompiledMachineBlockKind;
  raw: string;
}

const MACHINE_BLOCK_KINDS = new Set<CompiledMachineBlockKind>([
  "workflow",
  "tool_policy",
  "output_contract",
]);
...
const workflowBranchSchema: z.ZodType<CompiledWorkflowBranch> = z.object({
  id: z.string().min(1),
  tool: z.string().min(1),
...
const workflowDefinitionSchema: z.ZodType<CompiledWorkflowDefinition> = z.object({
  id: z.string().min(1),
  entry: z
    .object({
...
const toolRequiredArgsMap: Record<string, string[]> = {
...
export async function discoverSkillMarkdownFiles(
  rootDir: string
): Promise<string[]> {
...
export async function loadSkillFromFile(sourcePath: string): Promise<CompiledSkill> {
...
export async function loadAllSkillsFromFs(rootDir: string): Promise<CompiledSkill[]> {
  ...
  if (errors.length > 0) {
    throw new Error(
      `Failed to load ${errors.length} skill(s) from ${rootDir}:\n  - ${errors.join("\n  - ")}`
    );
  }
...
export function compileSkillMarkdown(input: {
  ...
  if (!workflowBlock) {
    throw new Error(
      `Skill ${frontmatter.name} at ${input.sourcePath} is missing a required yaml workflow block`
    );
  }

  const workflow = workflowDefinitionSchema.parse(
    parseYamlBlock(workflowBlock.raw, "workflow", input.sourcePath)
  );
  const toolPolicy = parseOptionalObjectBlock(
    blocks,
    "tool_policy",
    input.sourcePath
  );
  const outputContract = parseOptionalObjectBlock(
    blocks,
    "output_contract",
    input.sourcePath
  );
  ...
  const compiledSkill: CompiledSkill = {
    ...
    mode: (frontmatter.mode || "embedded") as CompiledSkillMode,
...
function extractNarrativeBody(markdownBody: string): string {
  return markdownBody
    .replace(
      /```ya?ml\s+(?:workflow|tool_policy|output_contract)[\s\S]*?\n```\s*/g,
      ""
    )
    .trim();
}

function parseOptionalObjectBlock(
  blocks: ParsedMachineBlock[],
  kind: Exclude<CompiledMachineBlockKind, "workflow">,
  sourcePath: string
  ...
  return genericObjectBlockSchema.parse(
    parseYamlBlock(block.raw, kind, sourcePath)
  );
}

function extractMachineBlocks(markdown: string): ParsedMachineBlock[] {
  const tokens = markdownParser.parse(markdown, {});
  const blocks: ParsedMachineBlock[] = [];
  ...
  for (const token of tokens) {
    ...
    if (!kind || !MACHINE_BLOCK_KINDS.has(kind as CompiledMachineBlockKind)) {
      continue;
    }

    blocks.push({
      kind: kind as CompiledMachineBlockKind,
      raw: token.content,
    });
  }
...
function parseYamlBlock(
  rawBlock: string,
  blockKind: CompiledMachineBlockKind,
  sourcePath: string
): unknown {
  try {
...
function validateCompiledSkillContract(skill: CompiledSkill): void {
...
function sourceMayRequireCloudMarket(source: unknown): boolean {
